<template>
  <Bento name="join" :areas="['logo', 'joinCard', 'join']" :columns="[35]">
    <To href="/"><Logo /></To>
    <Card style="grid-area: joinCard" height="10" width="35">
      <template #title></template>
      <template #icon>
        <SuggestiveInput
          v-model="adjective"
          :list="adjectives"
          placeholder="masmavi"
          persistent-focus
          @enter="selectAdjective"
        />
        <span>&nbsp;-&nbsp;</span>
        <SuggestiveInput
          ref="name"
          v-model="name"
          :list="names"
          placeholder="deniz"
          @enter="join"
        />
      </template>
    </Card>
    <div class="join-button">
      <Press badge @click="join">
        <span> join room </span>
      </Press>
    </div>
  </Bento>
</template>

<script lang="ts">
import { adjectives, names } from '~/config'
import type SuggestiveInput from '~/components/SuggestiveInput.vue'
export default defineComponent({
  data() {
    return {
      adjective: '',
      name: '',
    }
  },
  computed: {
    adjectives() {
      return adjectives
    },
    names() {
      return names
    },
  },
  methods: {
    selectAdjective({ suggestion }: { suggestion: string }) {
      this.adjective = suggestion
      // focus on next element
      const name = this.$refs.name as typeof SuggestiveInput
      name?.focus()
    },
    async join({ suggestion }: { suggestion: string }) {
      if (!this.adjective || !this.name) return
      // fill out the suggestion if it's not filled
      this.name = this.name.length ? suggestion : ''
      // check room exists
      const exists = await this.$scrumz.roomExists(
        `${this.adjective}-${this.name}`
      )
      if (!exists) {
        this.$toast.error('room does not exist')
        return
      }
      this.$to(`/room/${this.adjective}-${this.name}`)
    },
  },
})
</script>

<style lang="scss">
.join {
  height: 100%;
  display: flex;
  justify-content: center;
  align-items: center;

  .spacer {
    grid-area: spacer;
  }
  .join-button {
    grid-area: join;
    display: flex;
    justify-content: end;
  }
}

@media (max-aspect-ratio: 1/1) {
  .join {
    height: 100vh;
    .bento-grid {
      padding: 2rem;
      min-height: 100%;
      overflow: scroll;
      grid-template-areas: 'joinCard';
      grid-template-columns: 100%;
      grid-template-rows: 8rem 1fr 4rem 8rem;
      align-content: space-between;
    }
  }
}
</style>
